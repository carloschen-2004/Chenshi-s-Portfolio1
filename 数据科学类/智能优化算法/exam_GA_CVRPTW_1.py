...
class Sol():
    def __init__(self):
        ...
        self.Route_overD =None  #每条车辆路径对应的载重违背值
        self.Route_overT =None  #每条车辆路径对应的时间违背值
        self.obj = None         #目标函数值
        self.fit = None         #适应度值
...
def readExcel(filepath,model,capacity):
    ...
    model.fleet = list(range(model.number, model.number + model.vehicle_num)) #车辆编号
    

#函数：初始种群生成
def initialpop(model):
    temp_chrom = list(range(0, model.number + model.vehicle_num))   #初始模板染色体为[0,1,2,..,n,n+1,...n+k]
    ...
    for i in chrom:
        ...
        # 判断是否为车辆
            ...
            if current_route:
                ...
                #print("得到一条子路径：",current_route)
                current_route = []  # 清空当前客户列表，准备下一组客户
    # 确保最后一个路径也被添加
    if current_route:
        routes.append(current_route)
    return routes
...
def calviolations(route, model):
    ...
    for index, c in enumerate(route):
        #计算第一个客户
        if index == 0:
            travel_time = math.sqrt((prev.x_coord - model.customer[c].x_coord)**2 + (prev.y_coord - model.customer[c].y_coord)**2)/model.v
        ...
        #计算剩余客户
            ...
            # 计算从prev到当前客户的行驶时间
            travel_time = math.sqrt((model.customer[prev].x_coord - model.customer[c].x_coord)**2 + (model.customer[prev].y_coord - model.customer[c].y_coord)**2)/model.v
            arrival = max(prev_time+travel_time, model.customer[c].et)
            # 累加时间窗违背值
            if arrival > model.customer[c].lt:
                overT += arrival - model.customer[c].lt
...
#函数：适应度值计算：obj = dis + p_d*overD + p_t*overT ; (1)fit=1/obj;(2)fit=objmax - dis，选(2)
def calFit(model):
    objMAX = -float('inf')
    ...
    for sol in model.pop:          #对种群中的每一条染色体进行操作
        ...
        Route_overD = []                    #存储该车辆路径方案中每条路径的容量违背值
        Route_overT = []                    #存储该车辆路径方案中每条路径的时间违背值
        for route in Route:
            dis = caldistance(route,model) #计算得到dis
            overD, overT = calviolations(route,model) #计算得到overD and overT
        ...
        sol.Route_overD = Route_overD #存储该染色体每条路径的载重违背值
        sol.Route_overT = Route_overT #存储该染色体每条路径的时间违背值
        sol.obj = sum(Route_dis) + model.p_d*sum(Route_overD) + model.p_t*sum(Route_overT) #计算目标函数值
        if sol.obj > objMAX:
    ...
    while True:
        ...
        if father_index != mother_index:                  #确保两个父代不为同一条染色体
            ...
            if random.random() < model.pc:   #一定概率发生交叉
                cpoint1 = int(random.randint(0,model.number+model.vehicle_num-1))       #交叉点1位置索引
                cpoint2 = int(random.randint(cpoint1,model.number+model.vehicle_num-1)) #交叉点2位置索引
                ...
                for i in range(model.number+model.vehicle_num):                     #对父代2同样的操作
...
def mutation(model):
    ...
    while True:
        ...
        mpoint1 = random.randint(0,model.number+model.vehicle_num-1)  #随机选出变异点1
        mpoint2 = random.randint(0,model.number+model.vehicle_num-1)  #随机选出变异点2
